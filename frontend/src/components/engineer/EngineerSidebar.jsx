...
const navItems = [
  { label: "Dashboard", path: "/engineer", end: true },
  { label: "Assigned Complaints", path: "/engineer/complaints" },
  { label: "Resolved Complaints", path: "/engineer/resolved" },
];

function EngineerSidebar({ user, onClose, onLogout }) {
    ...
    <>
      <div className="border-b border-slate-200 px-5 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-slate-950">Engineer Portal</p>
            <p className="text-xs text-slate-500">{user?.name || "Engineer"}</p>
          </div>
        </div>
      </div>
